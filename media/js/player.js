(function($){
	...
	$("#QPlayer .ssBtn").on('click', function(){
		var mA = $("#QPlayer");
		if ($('.ssBtn .adf').hasClass('hidden') === false) {
			if (isFirstPlay) {
			...
			mA.css("transform", "translateX(-260px)");
		    $('.ssBtn .adf').addClass('hidden');
		} else {	
			mA.css("transform", "translateX(0)");
            $('.ssBtn .adf.hidden').removeClass('hidden') 	
		}
	}); 
